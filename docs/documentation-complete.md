...
1. Backend charge depuis MariaDB :
   ...
   └── User.aiPreferences → objectif_ia, allergies, regime, budget
...
3. AiNutritionService →
   ...
   {
     "userId": 42,
     "userGoal": "perte_de_poids",            ← aiPreferences.objectif_ia (fallback: "equilibre")
     "allergies": ["lactose"],                ← aiPreferences.allergies
     "dietaryConstraints": ["vegetarien"],    ← aiPreferences.regime
     "budget": 150.0,                         ← aiPreferences.budget (€/mois)
...
### 7.4 Flux — Analyse photo d'un repas

**Endpoint client :** `POST /ai/nutrition/analyze-photo` (JWT requis, multipart/form-data)  
**Implémenté dans :** branche `feat/ai-photo-food-detection` (backend)

```
1. Client envoie une photo JPEG/PNG dans le champ "photo" (max 10 Mo)

2. Backend valide le type MIME (JPEG ou PNG uniquement → 400 sinon)

3. StorageService.upload() → photo uploadée sur S3/MinIO → URL publique obtenue

4. AiNutritionService →
   POST {WORKOUT_SERVICE_URL}/ai/nutrition/analyze-photo
   Header : X-API-Key: {WORKOUT_SERVICE_API_KEY}
   Timeout : 30 secondes (vision IA locale plus lente que le NLP)
   Body :
   {
     "imageUrl": "https://s3.../photo.jpg",  ← URL S3/MinIO de la photo
     "userId": 42
   }

5. api-ia exécute la chaîne vision :
   Ollama Vision (llava/moondream) → Google Vision → stub
   Puis résolution des macros depuis MongoDB nutrition_foods
   Puis détection des déséquilibres nutritionnels

6. Backend persiste le résultat :
   INSERT AiNutritionRecommendation (
     user_id=42,
     type='ANALYSIS',
     input_image_url='https://s3.../photo.jpg',
     aliments_detectes={JSON},
     macros={JSON},
     suggestions={JSON}
   )

7. Retourne FoodAnalysisResult au client :
   { alimentsDetectes, macros, suggestions, imageUrl }
```
...
### 7.5 Points en cours / à venir

- **`limitations_physiques` et `preferences_sportives`** non transmis au micro-service workout sur `main` — corrigé sur `feat/ai-photo-food-detection` (branche Flo), ticket [#199](https://github.com/MSPR-c-l-w/backend/issues/199).
- **`objectif_ia` hardcodé** sur `main` — corrigé sur `feat/ai-photo-food-detection`, ticket [#198](https://github.com/MSPR-c-l-w/backend/issues/198). Ces deux tickets seront fermés au merge de la branche.
