...
- **Ré-entraînement périodique** : le volume réel de feedback augmentera avec l'usage en production. L'entraînement reste **par lot** (le volume ne justifiera pas un passage à l'online learning, cf. discussion §3.5) mais la **fréquence** doit devenir automatique plutôt que manuelle — brancher un job de réentraînement sur `EtlWeeklySchedulerService` (mécanisme de planification déjà existant pour les pipelines ETL, voir `documentation/architecture.md`), avec validation des métriques avant de remplacer le `.joblib` en production (éviter de déployer un modèle moins bon qu'avant sans s'en rendre compte).
