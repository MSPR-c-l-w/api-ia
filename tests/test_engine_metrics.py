...
def test_rss_equals_sum_squared_residuals(
    exercise_cardio, exercise_strength, profile_beginner
):
    from app.contexts.workout.domain.services.recommendation_engine import (
        score_exercise,
    )
    ...
    expected_rss = sum((yt - yp) ** 2 for yt, yp in zip(y_true, y_pred, strict=False))
