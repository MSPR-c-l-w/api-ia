...
class MealComposerService:
    ...
    def compose_week(
        ...
        for day in range(1, 8):
            ...
            # Mémorise les protéines/glucides utilisés pour varier les jours suivants
            for f in lunch.foods + dinner.foods:
                cat = next((i.category for i in self._items if i.name == f), None)
                if cat == FoodCategory.PROTEIN and f not in used_proteins:
                    used_proteins.append(f)
                elif cat == FoodCategory.CARB and f not in used_carbs:
                    used_carbs.append(f)
    ...
    def _compose_slot(
        ...
        for starter in starters:
            ...
            combined = _combine_macros(selected)
            score, devs = _score_meal(combined, target)
            candidate = ComposedMeal(
                foods=[s.name for s in selected],
                macros=combined,
                score=score,
                deviation_pcts=devs,
            )
